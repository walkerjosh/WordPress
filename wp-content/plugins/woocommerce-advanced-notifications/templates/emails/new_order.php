...
<table cellspacing="0" cellpadding="6" style="width: 100%; border: 1px solid #eee;" border="1" bordercolor="#eee">
	...
	<tbody>
		...
		foreach ( $order->get_items() as $item ) :
			...
			if ( $triggers['all'] || in_array( $item['id'], $triggers['product_ids'] ) || in_array( $_product->get_shipping_class_id(), $triggers['shipping_classes'] ) )
				$display = true;
			
			if ( ! $display ) {
			
				$cats = wp_get_post_terms( $item['id'], 'product_cat', array( "fields" => "ids" ) );
