 ...
 */
if ( is_admin() ) {
	$woo_plugin_updater_table_rates_2 = new WooThemes_Plugin_Updater( __FILE__ );
	$woo_plugin_updater_table_rates_2->api_key = '0650c47aef1e48190ed21c310096c698';
	$woo_plugin_updater_table_rates_2->init();
}
...
if ( is_woocommerce_active() ) {
	...
	define( 'TABLE_RATE_SHIPPING_VERSION', '2.1.3' );
		...
		class WC_Shipping_Table_Rate extends WC_Shipping_Method {
		    ...
		    function init_instance_settings() {
		    	...
		    	if ( ! empty( $this->instance_fields ) && ! empty( $this->instance_id ) ) {
			    	...
			    	$this->settings = array_merge( $this->settings, array_map( array( &$this, 'format_settings' ), $instance_settings ) );
		    	}
		    ...
		    function query_rates( $args ) {
				...
				if ( $shipping_class_id == "" ) {
					...
					$shipping_class_id_in = " AND rate_class IN ( '', " . (int) $shipping_class_id . " )";
				}
				...
				if ( $this->calculation_type == 'item' ) {
	    			...
	    			foreach ( $package['contents'] as $item_id => $values ) {
						...
						if ( $values['quantity'] > 0 && $_product->needs_shipping() ) {
							...
							$item_weight 		= round( $_product->get_weight() );
							...
							$costs[ $item_id ] = ( $item_cost + $item_fee ) * $values['quantity'];
	    			...
	    			foreach ( $package['contents'] as $item_id => $values ) {
						...
						if ( $values['quantity'] > 0 && $_product->needs_shipping() ) {
							...
							$item_weight 		= round( $_product->get_weight() * $values['quantity'] );
							...
							$costs[ $item_id ] = $item_cost + $item_fee;
	    			...
	    			foreach ( $package['contents'] as $item_id => $values ) {
	    				...
	    				if ( $values['quantity'] > 0 && $_product->needs_shipping() ) {
		    				...
		    				if ( ! isset( $classes[ $shipping_class ] ) )
		    					$classes[ $shipping_class ] = array();

		    				$classes[ $shipping_class ][ $item_id ] = $values;
	    			...
	    			foreach ( $classes as $class_id => $products ) {
						...
						foreach ( $matching_rates as $rate ) {
							...
							$total_cost += $rate->rate_cost_per_item * $count_in_class;
							$total_cost += $rate->rate_cost_per_weight_unit * $weight;
							$total_cost += ( $rate->rate_cost_percent / 100 ) * $price;
							if ( $rate->rate_priority )
									$stop = true;
							$matched = true;
						}

						$class_fee	= $this->get_fee( $this->fee, $price );
