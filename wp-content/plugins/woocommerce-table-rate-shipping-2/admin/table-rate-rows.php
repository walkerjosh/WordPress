...
function wc_table_rate_admin_shipping_rows( $table_rate_shipping ) {
	...
	jQuery('#woocommerce_table_rate_calculation_type').change(function(){
		...
		if ( selected ) {
			...
			jQuery( 'td.shipping_label, th.shipping_label, #shipping_class_priorities' ).hide();
			
		} else {
			
			jQuery( 'td.shipping_label, th.shipping_label, #shipping_class_priorities' ).show();
	...
	if (!$classes) :
		...
		<table class="widefat" style="position:relative;">
			...
			<tfoot>
				<tr>
					<td colspan="2">
						<span class="description"><?php _e('When calculating shipping, the cart contents will be <strong>searched for all shipping classes</strong>. If all product shipping classes are <strong>identical</strong>, the corresponding class will be used.<br/><strong>If there are a mix of classes</strong> then the class with the <strong>lowest number priority</strong> (defined above) will be used.', 'wc_table_rate'); ?></span>
					</td>
				</tr>
			</tfoot>
			<tbody>
