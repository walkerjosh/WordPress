...
class WC_Shipping_Zone_Methods_Table extends WP_List_Table {
        	...
        	case 'title' :
        		...
        		return '
        			...
        			<div class="row-actions">
        				<span class="id">ID: ' . $item->instance_id . ' | </span><span><a href="' . add_query_arg( 'method', $item->number, add_query_arg( 'zone',  $this->zone_id, admin_url( 'admin.php?page=shipping_zones' ) ) ) . '">' . __( 'Edit' , 'wc_table_rate' ) . '</a> | </span><span class="trash"><a class="submitdelete" href="' . wp_nonce_url( add_query_arg( 'delete_method', $item->number ), 'woocommerce_delete_method' ) . '">' . __( 'Delete', 'wc_table_rate' ) . '</a></span>
        			</div>';

        	case 'type' :
        		return $item->method_title;
        	case 'enabled' :
        		return ( $item->enabled == 'yes' ) ? '<img src="' . $woocommerce->plugin_url() . '/assets/images/success.png" alt="yes" />' : '&ndash;';
        	case 'rows' :
        		return sizeof( $item->get_shipping_rates() );
        }
	}
    ...
    function get_columns(){
        $columns = array(
            ...
            'type'     		=> __('Method Type', 'wc_table_rate'),
            'enabled'  		=> __('Enabled', 'wc_table_rate'),
            'rows'			=> __('Number of rates', 'wc_table_rate')
        );
        return $columns;
    }
    ...
    function process_bulk_action() {
        ...
        if ( 'delete' === $this->current_action() ) {
            ...
            echo '<div class="updated success"><p>' . __( 'Shipping methods deleted', 'wc_table_rate' ) . '</p></div>';
	 ...
	 */
	function no_items() {
		echo '<p>' . __( 'No shipping methods found.', 'wc_table_rate' ) . '</p>';
	}
