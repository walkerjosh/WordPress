...
class WC_Shipping_Zones_Table extends WP_List_Table {
    ...
    function column_default( $item, $column_name ) {
        ...
        switch( $column_name ) {
        	case 'zone_name' :
        		...
        		if ( $item->zone_id > 0 ) {
	        		...
	        		<div class="row-actions">
						<a href="' . add_query_arg( 'edit_zone', $item->zone_id, admin_url( 'admin.php?page=shipping_zones' ) ) . '">' . __( 'Edit', 'wc_table_rate' ) . '</a> | <a href="' . add_query_arg( 'zone', $item->zone_id, admin_url('admin.php?page=shipping_zones') ) . '" class="configure_methods">' . __('Configure shipping methods', 'wc_table_rate') . '</a>
					</div>';
				} else {
					$zone_name .='
	        		<div class="row-actions">
						<a href="' . add_query_arg( 'zone', $item->zone_id, admin_url('admin.php?page=shipping_zones') ) . '" class="configure_methods">' . __('Configure shipping methods', 'wc_table_rate') . '</a>
					</div>';
				}
        	...
        	case 'zone_type' :
        		...
        		if ( $item->zone_id == 0 )
        			return __( 'Everywhere', 'wc_table_rate' );
        		...
        		foreach ( $locations as $location ) {
        			if ( sizeof( $locations_list ) >= 8 ) {
	        			$locations_append = ' ' . sprintf( __( 'and %s others', 'wc_table_rate' ), ( $count - 8 ) );
	        			break;
        			}
	        		switch ( $location->location_type ) {
		        		...
		        		case "state" :
		        			...
		        			if ( $item->zone_type == 'postcodes' ) {
			        			$locations_prepend = sprintf( __( 'Within %s:', 'wc_table_rate' ), $location_name ) . ' ';
		        			} else {
        		...
        		switch ( $item->zone_type ) {
	        		case "countries" :
	        			return '<strong>' . __( 'Countries', 'wc_table_rate' ) . '</strong><br/>' . $locations_prepend . implode( ', ', $locations_list ) . $locations_append;
	        		case "states" :
	        			return '<strong>' . __( 'Countries and states', 'wc_table_rate' ) . '</strong><br/>' . $locations_prepend . implode( ', ', $locations_list ) . $locations_append;
	        		case "postcodes" :
	        			return '<strong>' . __( 'Postcodes', 'wc_table_rate' ) . '</strong><br/>' . $locations_prepend . implode( ', ', $locations_list ) . $locations_append;
        		}
        	case 'enabled' :
        		return ( $item->zone_enabled ) ? '<img src="' . $woocommerce->plugin_url() . '/assets/images/success.png" alt="yes" />' : '&ndash;';
        	case 'methods' :
    ...
    function get_columns(){
        $columns = array(
            ...
            'zone_name'     => __('Zone name', 'wc_table_rate'),
            'zone_type'     => __('Zone type', 'wc_table_rate'),
            'enabled'  		=> __('Enabled', 'wc_table_rate'),
            'methods'  		=> __('Shipping Methods', 'wc_table_rate'),
        );
        return $columns;
    }
    ...
    function get_bulk_actions() {
        $actions = array(
            'disable'   => __('Disable', 'wc_table_rate'),
            'enable'    => __('Enable', 'wc_table_rate'),
            ''   		=> '------',
            'delete'    => __('Delete', 'wc_table_rate')
        );
        return $actions;
    }
    ...
    function process_bulk_action() {
        ...
        if ( 'delete' === $this->current_action() ) {
            ...
            echo '<div class="updated success"><p>' . __('Shipping zones deleted', 'wc_table_rate') . '</p></div>';
        	...
        	echo '<div class="updated success"><p>' . __('Shipping zones enabled', 'wc_table_rate') . '</p></div>';
        	...
        	echo '<div class="updated success"><p>' . __('Shipping zones disabled', 'wc_table_rate') . '</p></div>';
    ...
    function prepare_items() {
		...
		$default->zone_id = 0;
		$default->zone_name = __('Default Zone (everywhere else)', 'wc_table_rate');
		$default->zone_type = __('All countries', 'wc_table_rate');
