...
class FUE {
    ...
    public static function create_order_from_triggers( $order_id = 0, $triggers = array() ) {
        ...
        foreach ( $items as $item ) {
            $email_results = $wpdb->get_results("SELECT DISTINCT `id`, `priority` FROM {$wpdb->prefix}followup_emails WHERE `interval_type` IN ($trigger) AND `product_id` = '". $item['id'] ."' AND `email_type` <> 'generic' AND `always_send` = 0 ORDER BY `priority` ASC");
            
            if ( $email_results ) {
                foreach ($email_results as $email) {
                    $emails[] = array('id' => $email->id, 'item' => $item['id'], 'priority' => $email->priority);
                }
            }
            
            // always_send product matches
            $results = $wpdb->get_results("SELECT DISTINCT `id` FROM {$wpdb->prefix}followup_emails WHERE `interval_type` IN ($trigger) AND `product_id` = '". $item['id'] ."' AND `always_send` = 1");
            
            foreach ( $results as $row ) {
                $always_prods[] = array( 'id' => $row->id, 'item' => $item['id'] );
            }
            
            // always_send category matches
            $cat_ids    = wp_get_object_terms( $item['id'], 'product_cat', array('fields' => 'ids') );
            $ids        = implode(',', $cat_ids);
            ...
            foreach ( $results as $row ) {
                $always_cats[] = array('id' => $row->id, 'item' => $item['id']);
            }
        }
        ...
        if ( !empty($always_cats) ) {
            foreach ( $always_cats as $row ) {
                ...
                $skip = false;
                do_action('fue_create_order_always_send', $email, $order_id);
        ...
        if ( !$order_created ) {
            ...
            foreach ( $items as $item ) {
                ...
                foreach ( $email as $e ) {
                    $emails[] = array('id' => $e->id, 'item' => $item['id'], 'priority' => $e->priority);
                }
            }
        ...
        foreach ($results as $email_order) {
            ...
            if ( $email->email_type == 'generic' ) {
                if ( $order ) {
                    ...
                    foreach ( $items as $item ) {
                        $item_list .= apply_filters( 'fue_email_item_list', '<li><a href="'. self::create_email_url( $email_order->id, $email->id, $user_id, $email_to, get_permalink($item['id']) ) .'">'. get_the_title($item['id']) .'</a></li>', $email_order->id, $item );
                ...
                if ( !empty($email_order->product_id) ) {
                    $item   = new WC_Product($email_order->product_id);
                    $cats   = get_the_terms($item->id, 'product_cat');
            ...
            $unsubscribe    = (empty($codes)) ? add_query_arg('wcfu', $email_to, get_permalink($page_id)) : add_query_arg($codes, add_query_arg('wcfu', $email_to, get_permalink($page_id)));
            ...
            if ( $email->email_type == 'generic' ) {
                ...
                $item_url   = (empty($codes)) ? get_permalink($item->id) : add_query_arg($codes, get_permalink($item->id));
                $item_url   = self::create_email_url( $email_order->id, $email->id, $user_id, $email_to, $item_url );
