...
jQuery( function() {
	...
	jQuery("form.checkout, form#order_review").on('change', '.card-number, .card-cvc, .card-expiry-month, .card-expiry-year, input[name=stripe_customer_id]', function( event ) {
		jQuery('.woocommerce_error, .woocommerce_message, .stripe_token').remove();
...
function stripeResponseHandler( status, response ) {
    ...
    if ( response.error ) {
        ...
        jQuery('.woocommerce_error, .woocommerce_message, .stripe_token').remove();
        jQuery('.card-number').closest('p').before( '<ul class="woocommerce_error"><li>' + response.error.message + '</li></ul>' );
        $form.unblock();
